...
def change_velocity_and_displacement(
    ...
    global_velocity[index] += tau*global_acceleration[index]
    global_displacement[index] += tau*global_velocity[index]

    global_acceleration[index] = 0

    if index == riker_pulse_node_id and step < riker_pulse.shape[0]:
...
def change_acceleration(
        ...
        global_mass: FloatN,
        global_weights: FloatExS2,
        global_nablas: FloatExS2xS2x2,   
        hooke: Float3x3,
    ) -> None:
    ...
    elem = global_elems[eid]

    elem_weights: FloatS2 = global_weights[eid]
    elem_nablas: FloatS2xS2x2 = global_nablas[eid]
    ...
    for mu in range(n_nodes_2d):
        for nu in range(n_nodes_2d):
            ...
            nabla_la = elem_nablas[nu, la]
            nabla_mu = elem_nablas[nu, mu]

            D_la = np.array([
                [nabla_la[0], 0], 
                [0, nabla_la[1]], 
                [nabla_la[1], nabla_la[0]]
            ])

            D_mu = np.array([
                [nabla_mu[0], 0], 
                [0, nabla_mu[1]], 
                [nabla_mu[1], nabla_mu[0]]
            ])

            global_acceleration[elem[la]] -= elem_weights[nu] / global_mass[elem[la]] * global_displacement[elem[mu]] @ D_la.transpose() @ hooke @ D_mu 
...
def simulation_loop(
    ...
    global_n_nodes = global_mass.shape[0]
    global_n_elems = global_elems.shape[0]

    for eid in range(global_n_elems):
        n_nodes_2d = global_elems[eid].shape[0]
    ...
    for nid in range(global_n_nodes):

        change_velocity_and_displacement(
            nid,
            tau,
            step,
            riker_pulse_node_id, 
